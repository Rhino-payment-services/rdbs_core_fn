        ...
        {!isLoading && error && (
          <div className="rounded-md border border-red-200 bg-red-50 p-4 flex gap-3">
            ...
            <div className="space-y-1 flex-1">
              ...
              <p className="text-xs text-red-700">{(error as any)?.response?.data?.message || error.message}</p>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-md border overflow-hidden">
              <div className="bg-gray-50 border-b px-4 py-2 flex items-center justify-between">
                <p className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Partner Response Body</p>
                <span className={`text-xs font-medium ${data.partnerResponse.success ? 'text-green-600' : 'text-red-600'}`}>
                  {data.partnerResponse.success ? 'Success' : 'Error'}
                </span>
              </div>
              <pre className="text-xs font-mono p-4 bg-white overflow-x-auto max-h-56 leading-relaxed text-gray-800 whitespace-pre-wrap">
                {JSON.stringify(data.partnerResponse, null, 2)}
              </pre>
            </div>
