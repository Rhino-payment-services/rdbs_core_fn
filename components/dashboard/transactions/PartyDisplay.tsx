...
export const PartyDisplay = ({ info, nameClassName, extras }: PartyDisplayProps) => (
  <>
    ...
    <span className={`text-xs font-medium ${
      info.type === 'MERCHANT' || info.type === 'SUBSCRIBER' || info.type === 'PARTNER' || info.type === 'PARTNER_INSTITUTION' ? 'text-blue-600' :
      info.type === 'ADMIN' ? 'text-purple-600' :
      'text-gray-500'
    }`}>
      {info.type === 'MERCHANT' ? '🏦 Merchant Account' :
       info.type === 'PARTNER_INSTITUTION' ? '🏛️ Partner Institution' :
